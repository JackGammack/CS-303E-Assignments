...
#  Description: Tutorial Exercises 10
...
def groupSumClump(start, nums, target):
    ...
    if( start+1<len(nums) and nums[start] == nums[start+1] ):
        n = 1
        while( start+n+1<len(nums) and nums[start] == nums[start+n+1] ):
            n += 1
        return groupSumClump(start+n+1,nums,target-(n+1)*nums[start]) or groupSumClump(start+n+1,nums,target)
    else:
...
def splitArrayHelper(start,nums,s1,s2):
    if (start>=len(nums)):
        return s1 == s2
    return splitArrayHelper(start+1,nums,s1+nums[start],s2) or splitArrayHelper(start+1,nums,s1,s2+nums[start])
...
def splitOdd10Helper(start,nums,s1,s2):
    if( start>=len(nums) ):
        if ( s1 % 10 == 0 and s2%2==1 ):
            return True
        elif ( s1%2==1 and s2%10==0 ):
            return True
        else:
            return False
    return splitOdd10Helper(start+1,nums,s1+nums[start],s2) or splitOdd10Helper(start+1,nums,s1,s2+nums[start])
...
def split53Helper(start,nums,s1,s2):
    ...
    if( nums[start]%5 == 0 ):
        ...
        return split53Helper(start+1,nums,s1+nums[start],s2) or split53Helper(start+1,nums,s1,s2+nums[start])

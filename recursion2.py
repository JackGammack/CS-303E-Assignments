...
#  Description: Tutorial Exercises 9
...
def changeXY(str):
    if ( str.find('x') == -1 ):
        ...
        return changeXY( str[0:str.find('x')] + 'y' + str[str.find('x')+1:len(str)])
...
def changePi(str):
    if ( str.find('pi') == -1 ):
        ...
        return changePi( str[0:str.find('pi')] + '3.14' + str[str.find('pi')+2:len(str)])
...
def noX(str):
    if ( str.find('x') == -1 ):
        ...
        return noX( str[0:str.find('x')] + str[str.find('x')+1:len(str)])
...
def allStar(str):
    if ( str.rfind('*') == len(str)-2 ):
        return str
    elif ( str.find('*') == -1 ):
        return allStar(str[0] + '*' + str[1:])
    else:
        return allStar(str[0:str.rfind('*')+2] + '*' + str[str.rfind('*')+2:])
...
def stringClean(str):
    if ( len(str)<=1 ):
        ...
        return str[1] + stringClean( str[2:] )
    else:
...
def nestParen(str):
    if ( len(str) == 0 ):
        return True
    elif ( len(str)%2 == 1  or str[0]!='(' or str[-1]!=')' ):
        return False
...
def strDist(str, sub):
    if ( str.find(sub) == -1 ):
        return 0
    elif( str.find(sub) == str.rfind(sub) ):
        return len(sub)
    else:
        return len(sub) + str.rfind(sub) - str.find(sub) - len(sub) + len(sub)
